...
export const SectionCountry = () => {
    ...
    const keyLenguages = Object.keys(country.languages)

    const currencies = keyCurrencies.map(key => country.currencies[key])
    const languages = keyLenguages.map(key => country.languages[key]).join(",")

    const linkLoacation = `https://www.google.com/maps/@${country.latlng[0]},${country.latlng[1]},10z?entry=ttu`

    const { code } = useParams()
    
    useEffect(() => {
        console.log(country.cca3, code)
    ...
    return(
        <StyledArticle>
            ...
            <ul>
                <li> <p> <b> Location : </b>
                <a href={linkLoacation} target ="_blank">{country.latlng[0]} , {country.latlng[1]}</a>
                </p> </li>
                <li> <p>
                    <b>Currencies : </b> 
